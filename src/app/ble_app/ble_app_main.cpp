...
void ble_app_main_setup( uint32_t tile_num ) {
    ...
    ble_app_main_tile = mainbar_get_tile_obj( tile_num );
    lv_style_copy( &ble_app_main_style, mainbar_get_style() );

    lv_obj_t * exit_btn = lv_imgbtn_create( ble_app_main_tile, NULL);
    lv_imgbtn_set_src(exit_btn, LV_BTN_STATE_RELEASED, &exit_32px);
    lv_imgbtn_set_src(exit_btn, LV_BTN_STATE_PRESSED, &exit_32px);
    lv_imgbtn_set_src(exit_btn, LV_BTN_STATE_CHECKED_RELEASED, &exit_32px);
    lv_imgbtn_set_src(exit_btn, LV_BTN_STATE_CHECKED_PRESSED, &exit_32px);
    lv_obj_add_style(exit_btn, LV_IMGBTN_PART_MAIN, &ble_app_main_style );
    lv_obj_align(exit_btn, ble_app_main_tile, LV_ALIGN_IN_BOTTOM_LEFT, 10, -10 );
    lv_obj_set_event_cb( exit_btn, exit_ble_app_main_event_cb );

    lv_obj_t * setup_btn = lv_imgbtn_create( ble_app_main_tile, NULL);
    lv_imgbtn_set_src(setup_btn, LV_BTN_STATE_RELEASED, &setup_32px);
    lv_imgbtn_set_src(setup_btn, LV_BTN_STATE_PRESSED, &setup_32px);
    lv_imgbtn_set_src(setup_btn, LV_BTN_STATE_CHECKED_RELEASED, &setup_32px);
    lv_imgbtn_set_src(setup_btn, LV_BTN_STATE_CHECKED_PRESSED, &setup_32px);
    lv_obj_add_style(setup_btn, LV_IMGBTN_PART_MAIN, &ble_app_main_style );
    ...
    lv_obj_t *app_label = lv_label_create( ble_app_main_tile, NULL);
    lv_label_set_text( app_label, "btapp");
    lv_obj_reset_style_list( app_label, LV_OBJ_PART_MAIN );
    lv_obj_add_style( app_label, LV_OBJ_PART_MAIN, &ble_app_main_style );
    lv_obj_align( app_label, ble_app_main_tile, LV_ALIGN_CENTER, 0, 0);

    // create an task that runs every secound
    _ble_app_task = lv_task_create( ble_app_task, 1000, LV_TASK_PRIO_MID, NULL );
}
